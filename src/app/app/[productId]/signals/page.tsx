...
export default async function SignalsPage({ params }: { params: Promise<{ productId: string }> }) {
  ...
  return <div className="flex flex-col gap-8"><PageHeader eyebrow="Primary analytics" title="Demand signals" description="One row is one demand cluster—not one URL. Expand a cluster to inspect provenance, matching rationale, and the action it suggests." actions={<><Select defaultValue="30"><SelectTrigger className="w-[150px]" aria-label="Filter by lookback"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="7"><LocalizedText text="Last 7 days" /></SelectItem><SelectItem value="30"><LocalizedText text="Last 30 days" /></SelectItem><SelectItem value="90"><LocalizedText text="Last 90 days" /></SelectItem></SelectGroup></SelectContent></Select><RunScanButton productId={productId} /></>} /><MetricStrip metrics={metrics} /><div className="flex flex-col gap-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[0.1em] text-muted-foreground"><LocalizedText text="Sorted by signal strength" /></p><p className="mt-1 text-sm text-muted-foreground"><LocalizedText text={`${clusters.length} clusters · ${totalSignals} independent signals · ${totalSources} sources`} /></p></div><span className="text-xs text-muted-foreground"><LocalizedText text="Live workspace data" /></span></div><SignalTable clusters={clusters} /></div></div>
}
